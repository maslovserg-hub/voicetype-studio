...
- **Установка:** `VoiceTypeStudio-Setup.exe` из [последнего релиза](https://github.com/maslovserg-hub/voicetype-studio/releases/latest).
- **Обновление:** в программе — «Транскриптор» → ⚙ Настройки → «Проверить обновления» → «Обновить».
- **Если кнопки ещё нет** (версия старее 1.0.3): скачайте `VoiceTypeStudio-Update.bat` из релиза и запустите двойным кликом — он сам закроет программу, скачает новую версию и запустит её.

Настройки, история и модель лежат отдельно (`C:\VoiceTypeStudio\data\`, `C:\gigaam_cache\`) и при обновлении не трогаются.
